...
async def setup_general(interaction: discord.Interaction, topic: Optional[Literal[
    ...
    guild_id = interaction.guild.id
    TRI_enablelist = [None, "bans warns channel", "confessions channel"]
    if guild_id == TRI_Archive:
...
async def setup_staff(interaction: discord.Interaction, topic: Optional[Literal[
    ...
    guild_id = interaction.guild.id
    TRI_enablelist = [None, "staff roles", "staff role", "staff ping"]
    if guild_id == TRI_Archive:
        if topic not in TRI_enablelist:
            return await interaction.response.send_message(f"**{topic}** is disabled for TRI Archive.")
    server_query = {"_id": str(guild_id)}
    ...
    async def select_callback(self, interaction, select):
        if interaction.guild.id == TRI_Archive:
            ...
            if manager:
                ...
                max_limit = 30 if is_adm(interaction.user) else 2
...
class TicketManager:
    ...
    async def create(self, thread: discord.Thread, creator: discord.Member, ticket_type: str):
        ...
        max_limit = 30 if is_adm(creator) else 2
        if open_count >= max_limit:
